...
data "aws_iam_role" "service_role" {
  ...
  depends_on = [resource.aws_iam_role.eks_service_role]
}
locals {
  k8s_version = "1.25"
}
...
resource "aws_eks_cluster" "control_plane" {
  ...
  provisioner "local-exec" {
    ...
    #command =  "Do { $response = Invoke-WebRequest -Uri '${self.endpoint}/healthz' -UseBasicParsing; } until ($response.StatusCode -eq 200)"
...
data "aws_iam_role" "node_role" {
  ...
  depends_on = [ resource.aws_iam_role.eks_node ]
}

# Create an IAM role for PowerUser
...
# Attach policies to the PowerUser role as needed

# Create an IAM role for ReadOnlyUser
...
locals {
  aws_auth_role_map = concat(
    ...
    [
    {
      rolearn  = aws_iam_role.power_user_role.arn
      username = "PowerUser"
      groups   = ["system:masters"]
    },
   ],
   [ {
      rolearn  = aws_iam_role.readonly_user_role.arn
...
resource "aws_eks_node_group" "node-ec2" {
  ...
  node_group_name = "SaayamNode_group"
  node_role_arn   = aws_iam_role.eks_node.arn
  subnet_ids      = concat(values(var.vpc_config.private_subnet_ids))
